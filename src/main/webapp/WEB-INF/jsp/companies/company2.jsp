...
<html>
	      ...
	      $(function() {
	    	  $('#addButton').click(function(){
	              var rowItem = $('<tr data-id='+(idCount++)+' style="cursor: pointer;">')
	                .addClass('rowItem')
	                .appendTo('#tbody_remn')
	              var colItem1 = $('<td data-field="currency">').appendTo(rowItem);
    ...
    $(function() {
      var currency = [];
	  $("select.firm_currencies option").each(function(){currency.push($(this).text())});
      $('table tr').editable({
        dropdowns: {
	...
	<body>
		...
		<c:forEach items="${currenciesList}" var="curs">
			<option value="${curs.alpha}">${curs.alpha} ${curs.currency_rus}</option>
		</c:forEach>
		</select>
		</div>
		<ul id="breadcrumbs">
		...
		<p><a href="${clientsPath}"><span class="cln icon" ></span>(${countClients})</a>
        <a href="${currenciesPath}"><span class="currency icon" ></span>(${countCurrencies})</a>
